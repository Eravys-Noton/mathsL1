...
\begin{definition}[Application linéaire]
    Soient $E, F$ des $\K$-espaces vectoriels et $f : E \to F$.
    $f$ est une \emph{application linéaire} si et seulement si $\forall x_1, x_2 \in E,\ \forall \lambda \in \K$ :
...
\begin{definition}
    ...
    \[ \ker(f) \coloneqq \{ x \in E : f(x) = 0_E \} \] 
    On définit l'\emph{image} de $f$, notée $\Im(f)$ telle que :
    \[ \Im(f) \coloneqq \{ y \in F : \exists x \in E,\ y = f(x) \} \]
\end{definition}

\begin{definition}
    \par \noindent Soient $E, F$ des espaces vectoriels et $f$ une application linéaire  de $E$ vers $F$.
    \begin{enumerate}
...
\begin{proof}
    \begin{enumerate}
        ...
        \begin{align*}
            ...
            &\iff p(x) = p(p(x))
        \end{align*}
        \item Posons $s(x) = 2p(x) - id_E(x)$. Montrons que $s(s(x)) = id_E(x)$ si et seulement si $p(p(x)) = p(x)$.
        \begin{align*}
            s(s(x)) &= s(2p(x) - id_E(x)) \\
...
\begin{definition}
    $\operatorname{O}(n) \coloneqq \{ R \in \mathcal{M}_n(\R) | \forall x, y \in \R^n : \langle Rx|Ry \rangle = \langle x|y \rangle \}$
\end{definition}
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \[ \exists P, Q \in \operatorname{GL}(n),\ B = Q A P^{-1} \]
