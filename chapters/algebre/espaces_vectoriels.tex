...
\begin{definition}[Magma]
	On appelle \textbf{magma} un ensemble $M$ muni d'une loi de composition \textbf{interne} \og $*$ \fg. On note $(M, *)$.
\end{definition}
...
\begin{definition}[$\K$-espace vectoriel]
	Soit $\K$ un corps. 
	\\
	Un $\K$-espace vectoriel est un ensemble $E$ composé d'une loi de composition \textbf{interne} \og $+$ \fg et une loi de composition \textbf{externe} \og $\cdot$ \fg telles que :
	\begin{align*}
		+ : E^2 &\to E \\
		(x, y) &\mapsto x + y
	\end{align*}
...
\begin{definition}
	...
	\begin{enumerate}
		...
		\begin{align*}
			\forall (\lambda_1, \ldots, \lambda_n) \in \K^n,\ u_i \in E,\ \sum_{i = 1}^n \lambda_i u_i = 0_E \implies \lambda_1 = \cdots = \lambda_n = 0
		\end{align*}
