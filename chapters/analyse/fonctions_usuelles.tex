...
\begin{proposition}
	Pour tout $a \in \R$, la dérivée de la fonction $x^a$ est $ax^{a-1}$.
\end{proposition}
...
\begin{proposition}
	...
	\begin{enumerate}
		\item $\cos'(x) = -\sin(x)$. 
		\item $\sin'(x) = \cos(x)$.
		\item $\forall x \in \R \backslash \left\{\frac{k\pi}{2}\right\}$ : $\tan'(x) = \frac{1}{\cos^2(x)} = 1 + \tan^2(x)$.
	\end{enumerate}
\end{proposition}
...
\begin{definition}
	On définit $\arccos,\ \arcsin$ et $\arctan$ comme étant les \textbf{bijections réciproques} des fonctions $\cos,\ \sin$ et $\tan$.
\end{definition}
